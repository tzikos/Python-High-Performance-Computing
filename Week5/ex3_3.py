...
def generate_mandelbrot_set_chunks(points, num_processes, chunk_size=1000):
    ...
    escape_times = pool.apply_async(compute_chunk, chunks)
    
    pool.close()
    pool.join()
    end_time = time.perf_counter()
    return escape_times, end_time - start_time
...
if __name__ == "__main__":
    ...
    for num_proc in tqdm(process_counts):
        _, elapsed_time = generate_mandelbrot_set_chunks(points, num_proc)
